...
# 分镜大纲（storyboard_outline）：严格 JSON
STORYBOARD_OUTLINE_SYSTEM = (
    "你是专业的视频分镜专家。严格按用户消息中的 JSON 格式输出，不要输出其他内容。"
)

# 定妆照 prompt 生成（lookbook_prompts）：严格 JSON
LOOKBOOK_PROMPTS_SYSTEM = (
    "你是 AI 视觉导演。严格按用户消息中的 JSON 格式输出，不要输出其他内容。"
)

# 分镜素材图生成（material_generate）：严格 JSON
MATERIAL_GENERATE_SYSTEM = (
    "你是 AI 视觉导演，擅长结合剧本上下文与参考素材，产出可直接用于生图模型的 prompt。"
    "严格按用户消息中的 JSON 格式输出，不要输出其他内容。"
)

# 分镜提示词生成（segment prompt）：纯文本输出
SEGMENT_PROMPT_SYSTEM = (
    "你是专业的视频生成提示词工程师。严格遵循用户消息中的提示词生成规范，只输出最终提示词正文。"
)
