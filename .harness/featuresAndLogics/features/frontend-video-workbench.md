...
摘要: 视频生成工作流的前端工作台：路由 / 的 WorkflowPage 以 keep-alive 方式组装 4 步组件（选集/分镜大纲/分镜管理/生成视频），sessionStore/workflowStore 双状态层，SSE 观流 + 3 秒轮询驱动异步生成，配套素材图选择/AI 生成/@ 引用输入三个 material 弹窗组件。
tags: 前端, 视频工作流, 页面组件, zustand, 轮询, SSE
---
...
视频生成工作流的前端入口（App.tsx 路由 `/` → WorkflowPage）：以 4 步向导形态驱动后端视频会话（select_episode → storyboard_outline → segment_management → generate_videos）。页面把 4 个步骤组件全部常驻挂载、非当前步骤仅 `display:none` 隐藏（keep-alive），保证切步骤不断开 SSE 观流、不丢流式/生成进度。
...
异步交互两种模式：Agent 类任务（分镜大纲生成、素材图生成、分镜提示词生成）走 run_id + `AgentRunProgress` SSE 观流；视频生成为后台任务 + `usePolling` 3 秒轮询会话详情。

后端链路与数据契约见「视频创作工作流」文档；AgentRunProgress/MindmapView/LazyVideo/usePolling/imageSrc 等公共设施见「前端公共设施」文档，本文不重复展开。

## 核心代码路径

- `frontend/src/pages/WorkflowPage.tsx` — 工作台页面：URL session_id 同步、current_step → workflowStore 同步、4 步组件 keep-alive 组装
- `frontend/src/components/workflow/StepNavigator.tsx` — 步骤导航（antd Steps，4 步标题与说明）
- `frontend/src/components/workflow/StepSelectEpisode.tsx` — 步骤 1：选剧本会话 + 分集卡片 + 视频参数表单（含重新选集级联确认）
- `frontend/src/components/workflow/StepStoryboardOutline.tsx` — 步骤 2：分镜大纲生成（run_id 观流）、导图预览/编辑双模式、重新生成级联确认
- `frontend/src/components/workflow/StepSegmentManagement.tsx` — 步骤 3：左右分栏的分镜配置（分镜形式/overlap/参考素材图/提示词生成）
- `frontend/src/components/workflow/Step6Videos.tsx` — 步骤 4：视频生成/停止/重新生成/恢复备份 + 最终视频与 timeline_data 展示
...
- `frontend/src/api/client.ts` — stepApi 全部步骤端点（select-episode、storyboard-outline、storyboard-segments、material-pool、videos 系列）
...
- 4 步常量：从剧本选集 / 分镜大纲 / 分镜管理 / 视频（与 types STEP_NAMES 对应）。
...
### 5. 步骤 2 — StepStoryboardOutline

- 数据：`step_results.storyboard_outline.result_data` 的 mindmap（markdown 导图）、segments（StoryboardSegment[]）、segment_count、edited 标记。
- 导图拼接 `buildMindmap(withDuration)`：逐行扫描 mindmap，`### ` 开头的行按序对应 segments[i]，把分镜内容（outline）压成单行 `- ` 列表子节点；withDuration=true（预览）时标题行追加 `Ns · overlap Ns` 后缀（overlap 仅 index>0 且 >0 时显示），编辑态不加后缀。预览用 `MindmapView`（height 520）渲染。
- 生成：`stepApi.generateStoryboardOutline(session_id, extra_prompt?)` 返回 run_id → `AgentRunProgress` 观流；done 且 success 时 `refreshSession()` 并回到预览态。已有大纲时「重新生成」先弹补充要求 Modal（内含 9 条 PRESET_PROMPTS 快捷下拉，选中以「；」拼接到输入框后自身回位），确认后再 Modal.confirm 警告「将清空分镜配置、已生成的分镜提示词与视频数据」；无大纲（上次失败）则直接执行/重试。
...
- 分镜提示词生成（仅全能参考模式，按钮 disabled 否则）：先 `stepApi.getSegmentPromptContext(session_id, index)` 拉上下文（story_outline/episode_context/当前分镜/参考素材图/overlap 与衔接规则）在 Modal 内逐块预览；确认后 `stepApi.generateSegmentPrompt(session_id, index)` 返回 run_id → AgentRunProgress 观流，生成中不允许关闭弹窗；成功后关弹窗 + refreshSession。已生成提示词可一键复制（navigator.clipboard）。
- 完成分镜配置：`stepApi.completeSegmentManagement(session_id)` 解锁第 4 步；完成前页面底部 Alert 提示「修改分镜配置会清空该分镜已生成的提示词并回退完成状态」。
...
- 生成中判定 `isGenerating = hasPendingVideos || stepResult._generating === true || loading`；`usePolling(refreshSession, {interval: 3000, enabled: hasPendingVideos || isBackgroundGenerating})`。
- 首次生成：Modal.confirm 预览（总数量/预计总时长/平均时长/分段列表/ComfyUI 整段生成说明）→ `stepApi.generateVideos` → message.loading + refreshSession 触发轮询。
- 结果视图：标题态（生成中/已停止/完成 + 计数与失败 Tag）；操作区「停止生成」（Popconfirm → cancelVideos，`_cancelled` 时隐藏）、「恢复备份」（hasBackup = `_backed_up_count > 0` 或任一视频含 `_old_video_path`，且非生成中 → restoreVideosBackup）、「重新生成」（确认弹窗 → regenerateVideos）。
- 最终视频卡（final_video.video_path 且非「生成失败」前缀）：LazyVideo（placeholderHeight 320）+ 下载按钮 + mock 演示标记 + Descriptions（分段数量/段间 overlap/参考图 timeline_data.images 数/参考音频 audios 数/任务 prompt_id 可复制）+ Collapse 展示 timeline_data JSON。
- 分段视频卡网格：getVideoStatus 优先 task_status（completed/pending/failed/cancelled），无 task_status 时按 video_path 兼容判断（无路径→pending、「生成失败」前缀→failed、否则 completed）；pending 显示 Spin 占位、cancelled 显示已取消占位、failed 显示失败占位。
- 本地路径展示统一 `getMediaSrc`：http(s) 原样、相对路径补 `/` 前缀（与 utils/imageSrc.ts 同语义的组件内副本）。

## 依赖与复用关系

- 依赖: antd ^6.2.2（Steps/Card/Modal/Segmented/Slider/Tabs/Upload/Alert 等）、react-router-dom ^7（路由与 URL 操作）、zustand ^5（sessionStore/workflowStore）、axios（client.ts）；公共设施 AgentRunProgress（SSE 观流）、MindmapView（导图）、LazyVideo（视频懒加载）、usePolling（轮询）、imageSrc（图片路径）；后端 `/api/v1/steps`、`/api/v1/sessions`、`/api/v1/agent-runs` 系列端点
...
- **keep-alive 副作用**：4 步组件常驻挂载（display:none 隐藏），切换步骤不卸载——SSE 观流与本地草稿态（如 descDrafts）跨步骤保留；也因此 MindmapView 需要 ResizeObserver 零尺寸守卫（display:none 下渲染，见「前端公共设施」文档）。
...
- **完成提示文案为旧流程残留**：Step6Videos 全部完成后仍显示「您可以下载各个视频片段，或使用视频编辑软件进行合并」，系分段生成时代遗留文案；当前链路以 final_video 整段视频为准。
- **getMediaSrc 与 imageSrc 重复**：Step6Videos 组件内 getMediaSrc 与 `frontend/src/utils/imageSrc.ts` 逻辑相同（http(s) 原样、其余补 `/`），未复用工具函数。
- **预计耗时为粗估**：初始态约 `totalSegments × 1.5` 分钟、重新生成弹窗约 `videos.length × 1.5` 分钟，均为前端写死的估算文案。
