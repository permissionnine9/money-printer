---
name: 剧本定妆照生成
摘要: 剧本工作流第 4 步（lookbook_images）：Agent 单轮产出英文生图 prompt 后走确定性生图状态机（间隔提交+并发轮询），为人物/场景实体生成定妆照，支持单张重生成/删除与手动完成步骤。
tags: 剧本工作流, 定妆照, 生图, AgentSDK, SQLite, 前端组件
---
...
剧本创作工作流（4 步）的第 4 步 `lookbook_images`：为分集设计中注册的人物（character）/场景（scene）实体生成"定妆照"（全剧统一的视觉锚点图）。设计为「Agent 出 prompt + 确定性生图」两段式——先用 Agent SDK 单轮调用把实体设定翻译成英文生图 prompt（结合剧本故事逻辑确定统一视觉风格），再用 ImageService 按状态机逐张提交生图任务并轮询回写。

关键特征：

- **实体类型限制**：仅 `character`/`scene` 可生成定妆照，线索（clue）/伏笔（foreshadow）无视觉形象被拒绝。
...
整体数据流：前端 StepLookbook 卡片弹窗 → scriptStepApi（axios `/api/v1`）→ backend/api/v1/script_sessions.py 定妆照路由 → start_agent_run（deps.py → AgentRunRegistry，返回 run_id）→ ScriptWorkflow.generate_lookbook / regenerate_lookbook_image → Agent 出 prompt（lookbook_prompts.md 模板）→ ImageService（submit_image_task + poll_i2i_task）→ ScriptManager 更新 lookbook_images 表 → WorkspaceStore.set_entity_lookbook 回写实体文件 → 前端 2s 轮询刷新。

## 核心代码路径

- `backend/api/v1/script_sessions.py` — API 层：5 条定妆照路由（generate/complete/list/regenerate/delete，约 419-469 行），生图路由前置校验 `_require_default_image_model`
- `backend/core/agents/script_workflow.py` — 工作流核心：`generate_lookbook`（约 606 行）/`regenerate_lookbook_image`（约 720 行）/`complete_lookbook`（约 754 行）/`_load_story_logic`；模块常量 `LOOKBOOK_VIDEO_PARAMS`（约 30 行）
- `backend/prompts/lookbook_prompts.md` — Agent prompt 模板（category: script, step: 5）：AI 视觉导演两步法（定风格 → 按风格语法写 90-150 英文单词 prompt），输出严格 JSON 数组
- `backend/core/agents/system_prompts.py` — `LOOKBOOK_PROMPTS_SYSTEM`（约 18 行）：严格 JSON 输出约束的 system prompt
- `backend/core/persistence/script_manager.py` — 持久层：`lookbook_images` 表建表（约 62 行）与 insert/update/get/list/delete 五个方法
- `backend/core/persistence/workspace_store.py` — `set_entity_lookbook`（约 528 行）：实体文件 frontmatter 回写定妆照引用
- `backend/core/services/image_service.py` — 生图服务：`build_image_service_from_model_config`（约 25 行）/`submit_image_task`（约 108 行）/`poll_i2i_task`（约 247 行）
- `frontend/src/components/script/StepLookbook.tsx` — 前端步骤组件（358 行）：人物/场景分组卡片、生成/重生成弹窗、2s 轮询、完成本步骤
- `frontend/src/api/client.ts` — scriptStepApi 定妆照 5 方法（约 418-458 行）
- `frontend/src/types/index.ts` — `LookbookImage` TS 类型（约 95 行）
...
| 路由 | 处理函数 | 行为 |
|------|---------|------|
| POST /{session_id}/lookbook/generate | generate_lookbook | 生图模型前置校验 → `start_agent_run("lookbook_images", factory)` 返回 `{run_id}` |
| POST /{session_id}/lookbook/complete | complete_lookbook | 调 workflow.complete_lookbook 手动标记完成，返回 `{completed: True}` |
| GET /{session_id}/lookbook?entity_id=&task_status= | list_lookbook | 按会话（可加实体/状态过滤）列定妆照，`{images, total}` |
| POST /{session_id}/lookbook/{image_id}/regenerate | regenerate_lookbook_image | 单张重生成 → `start_agent_run(f"lookbook_regen_{image_id}")` |
| DELETE /{session_id}/lookbook/{image_id} | delete_lookbook_image | ScriptManager.delete_lookbook，消息 `定妆照已删除`/`定妆照不存在` |
...
- 请求 schema（backend/schemas/script.py 约 80-88 行）：`LookbookGenerateRequest{entity_ids: list[str] (min_length=1), style_prompt: str = "", model_config_id: Optional[str]}`；`LookbookRegenerateRequest{prompt: Optional[str], model_config_id: Optional[str]}`。
- `start_agent_run`（backend/deps.py 约 90 行）统一提交到 `get_run_registry()`（backend/core/agent_sdk/registry.py）并返回 `{"success": True, "data": {"run_id": ...}}`。
...
2. **Agent 出 prompt**：`_load_story_logic(session_id)[:1500]`（工作区文件优先，DB 老数据 fallback 到 story_ideation 步骤结果）；`self.prompts.render("lookbook_prompts", {story_logic, style_prompt, entities})` 渲染模板；`run_agent(AgentRunOptions(prompt=..., system_prompt=LOOKBOOK_PROMPTS_SYSTEM, max_turns=2, interrupt=...))` 单轮调用；`extract_json_array(result.text)` 解析 JSON 数组。
3. **缺漏兜底**：以勾选集为准，Agent 漏掉的实体用兜底 prompt：`"{name}, {description}, single subject, consistent character design, medium shot"`。
4. **确定性生图**（沿用素材图「间隔提交+并发轮询」模式）：
   ...
   - 逐张 `submit_image_task(prompt, LOOKBOOK_VIDEO_PARAMS, None)`，两张之间 `await asyncio.sleep(IMAGE_REQUEST_TIME_GAP)`（backend/core/config.py，12 秒，防生图服务限流）；提交成功 → `{task_id, task_status: processing}`，失败/异常 → task_status=failed；interrupt 置位抛 `已取消`。
...
校验 `row["script_session_id"] == session_id`（否则抛 `定妆照不存在`）→ update task_status=processing（可选同时改写 prompt）→ submit + poll 单张 → 成功回写 image_path 与实体引用，返回最新行。与批量路径不同：此路径生图失败/提交失败直接抛 ScriptWorkflowError（run 流结束报错），而非静默置 failed。
...
### Prompt 模板（backend/prompts/lookbook_prompts.md）
...
- **第二步写 prompt**：英文一段式自然语言 90-150 词（面向 GPT Image 类模型）；主体特征开头、写死固定特征（人物核心服装发型 / 场景标志元素）、含景别构图、单主体画面（人物不与别人同框、场景无人物）；按风格分四套语法（真实摄影语言 / anime 作画语言 / 渲染语言 / 其他媒介术语），写实风格禁用 `cinematic lighting`/`8k`/`masterpiece` 等 AI 味词。
...
- `list_lookbook` 按 `created_at ASC` 排序，支持 entity_id / task_status 过滤。
- 下游清空：`delete_script_data(script_session_id)`（约 234 行，大纲重生成时清下游）统计并删除本会话全部 lookbook_images 行。
...
- **单卡片状态**：`imageByEntity`（useMemo）取实体当前定妆照行——优先 `entity.lookbook_image_id` 指向的行，否则该实体最新一条（rows[rows.length-1]）。展示态：completed 且有 image_path → Image（imageSrc 处理：http(s) 原样，本地相对路径补 `/`）；pending → 「排队中」Spin；processing → 「生成中」Spin；failed → 「生成失败」；无 → 「暂无定妆照」。
...
- 被依赖: 定妆照是后续分镜/视频工作流的视觉锚点——`backend/core/services/script_context_service.py` 的 `fetch_lookbook_images`（约 115 行）直查 `list_lookbook(task_status="completed")` 并以 `lookbook_{image_id}` 形态并入分镜素材池（image_type='lookbook'）；`backend/core/agents/storyboard.py`（约 374 行）将定妆照作为独立素材组（label「定妆照」）供分镜引用（素材池 ID 规则 `lookbook_lb_*`，见 backend/core/models/video_models.py）；实体卡上下文对已有定妆照的实体标注「已有定妆照，勿重复生成」（script_context_service.py 约 108 行）。实体 frontmatter 的 lookbook_image_id/path 引用主要供前端 StepLookbook 展示与 DB→workspace 迁移脚本（backend/scripts/migrate_db_to_workspace.py）使用
- 可复用组件: 「间隔提交+并发轮询」生图状态机模式与分集素材图（episode_material_images）共用设计；`_require_default_image_model` 前置校验被多条生图路由复用
