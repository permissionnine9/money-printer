---
name: 前端公共设施
摘要: 跨页面复用的前端公共层，涵盖会话侧栏分组展示、主布局双轨会话管理与全局后台任务坞（AgentRunDock/agentRunStore/useRunTask/RunTaskBanner）、Agent 运行进度 SSE 流式消费与断线重连、提示词查看弹窗、思维导图渲染、视频懒加载、带次数上限的轮询 hooks、统一 API 客户端与 SSE 请求工具。
tags: 前端, 公共组件, hooks, SSE, API客户端, 全局任务跟踪, zustand
---
...
视频生成工作流与剧本创作两条业务线共用的前端基础设施：`SessionSider` 会话列表侧栏（平铺/二级分组两种形态）、`MainLayout` 布局骨架与双轨会话状态管理（全局挂载 `AgentRunDock` 后台任务坞，顶栏提供远程 ComfyUI 连接信息入口）、`agentRunStore` + `useRunTask` + `RunTaskBanner` 组成的**全局 Agent 任务跟踪设施**（跨菜单/跨步骤跟踪生成任务，SSE 观流不因切换页面而断）、`AgentRunProgress` 对 Agent 运行事件的 SSE 流式消费（断线重连 + seq 续传 + 取消）、`PromptViewerModal` 提示词查看/复制、`MindmapView` markdown 思维导图渲染、`LazyVideo`/`useLazyLoad` 视口懒加载、`usePolling` 轮询（含 maxPolls 次数上限兜底）、`client.ts` axios 实例与各业务 API、`sse.ts` 的 `fetchSSE` 流式请求工具。这些设施由 `frontend/src/components/common/index.ts` 统一导出（组件部分），供各页面与步骤组件复用。

术语约定：「核心素材图」即原「定妆照」；「素材库」（lookbook library）指跨剧本复用已完成核心素材图的能力（见 lookbook-library 文档）。

## 核心代码路径

- `frontend/src/components/common/SessionSider.tsx` — 会话侧栏组件，支持平铺与按 group.key 二级分组两种形态（grouped prop + SessionGroupMeta + order 排序键）
- `frontend/src/components/layout/MainLayout.tsx` — 全局布局（Header/Sider/Content/Footer）+ 视频/剧本双轨会话状态管理 + 全局挂载 AgentRunDock + 顶栏 ComfyUIConnectionModal 入口
- `frontend/src/components/layout/AgentRunDock.tsx` — 全局后台任务坞：悬浮 Badge 按钮 + 任务卡片列表（每 run 保活观流，收起不卸载）
- `frontend/src/stores/agentRunStore.ts` — zustand 全局 Agent 任务状态（runs: AgentRunTask[]）
- `frontend/src/hooks/useRunTask.ts` — 全局任务 hooks：guardRunStart（入队守卫）/ useRunActive / useRunError
- `frontend/src/components/common/RunTaskBanner.tsx` — 任务状态横幅（绿色进行中/红色失败，纯展示三 props）
...
- `frontend/src/hooks/usePolling.ts` — 轮询 hook（savedCallback ref 模式 + maxPolls/onMaxPolls 次数上限）
- `frontend/src/api/client.ts` — axios 实例（baseURL `/api/v1`）+ sessionApi/stepApi/modelApi/promptApi/uploadApi/settingsApi/scriptSessionApi/scriptStepApi/entityApi/agentRunApi（assetApi 已删除）
- `frontend/src/api/sse.ts` — `fetchSSE` 流式请求工具（GET/POST 自适应、按空行分帧解析）
- `frontend/src/components/common/index.ts` — 公共组件统一导出（LazyVideo、MindmapView、PromptViewerModal、RunTaskBanner、SessionSider）
...
- `sessions: Array<Pick<Session,'session_id'|'status'|'completed_steps'|'updated_at'> & {title?, group?: SessionGroupMeta, order?: number}>`（`SessionGroupMeta = {key: string; title: string}`）
- 其余 props：`grouped?`、`loading`、`currentSessionId?`、`totalSteps`、`onCreate`、`creating?`、`onSelect(sessionId)`、`onDelete(sessionId)`、`onRefresh`

两种模式：

- `grouped=true`（视频页）：按引用剧本二级分组——组头 = 剧本名，二级条目 = 「第N集·集名」按集号（order）降序；无引用剧本的会话归入哨兵组 `UNGROUPED_KEY='__ungrouped__'`。
- `grouped=false`（剧本页）：平铺显示剧本名。

分组派生：

- `grouped=true` 时按 `s.group.key` 用 `Map` 派生二级分组，插入序即后端 `updated_at DESC` 序；无 `group` 的会话归入哨兵组，组头标题固定「未选择剧本分集」。
...
### 2. MainLayout — 布局、双轨会话管理与全局任务坞挂载

- 结构：antd Layout：Header（标题「AI视频创作智能体」+ Menu 水平导航：`/script` 创作剧本、`/` 视频生成工作流、`/models` 模型管理、`/prompts` 提示词管理 + **ComfyUIConnectionModal 入口**：CloudServerOutlined 云服务器图标，查看/手动录入远程 ComfyUI SSH 连接信息——settingsApi.getComfyUIConnection/updateComfyUIConnection，60s timeout）+ 条件渲染 Sider width=280（仅 pathname 为 `/` 或 `/script` 时）+ Content{children} + **`<AgentRunDock />`（Content 之后全局挂载，后台任务坞）** + Footer。
- 状态：视频/剧本双轨各自 useState（sessions 列表、loading、creating）；当前会话来自 zustand `useSessionStore`/`useScriptSessionStore` 的 currentSession。
- **SPA 内会话切换**：navigateWithSession（整页刷新跳转）**已废弃**——切换会话直接调 `store.loadSession(id)` 写 store（选中相同 id 直接 return），页面随 currentSession 重渲染并同步 URL；删除当前会话调 `setCurrentSession(null)` + `navigate(pathname,{replace:true})` 显示欢迎引导（清 URL session_id），列表刷新由 currentSession 变化触发的 effect 完成；删除非当前会话则手动调 `loadVideoSessions`/`loadScriptSessions`。
...
- 侧栏数据组装：`videoSessionItems`（视频页 grouped：title=`第N集·集名` 或 episode_title，order=episode_number，group={key: script_session_id, title: script_title||'未命名剧本'}）与 `scriptSessionItems`（剧本页平铺：title=剧本名||'未命名剧本'）；两处 totalSteps 均传 4。

### 3. AgentRunDock — 全局后台任务坞

- 挂载于 MainLayout（Content 之后），全路由可见；有任务（runs 非空）时右上角显示悬浮 Badge 按钮，点击展开任务卡片列表。
- 每个 run 渲染一个任务卡片（Modal 形态）内嵌 `AgentRunProgress`，并以 forceRender 保活——**收起不卸载组件，SSE 观流不断流**。
- 成功收尾：按 kind 弹提示并 `removeRun`；run 的 sessionId 与当前会话匹配时刷新 scriptSessionStore/sessionStore。失败收尾：卡片标红、留在列表。

### 4. agentRunStore — 全局任务状态（zustand）

- `runs: AgentRunTask[]`；任务字段：`runId` / `sessionId` / `kind` / `segmentIndex` / `segmentTitle` / `episodeId` / `status: 'queued'|'running'|'success'|'error'` / `expanded`。
- kind 四类：`segment_prompt` / `outline` / `episodes` / `storyboard_outline`（步骤组件入队时约定）。
- 方法：`addRun`（新任务默认展开、其余全部收起）/ `expandRun` / `collapseRun` / `markRunRunning`（started 事件驱动 queued→running）/ `markRunStatus` / `removeRun`。
- 派生：`hasRunningRun` / `hasRunningSegmentPrompt`。

### 5. useRunTask / RunTaskBanner — 任务跟踪 hooks 与横幅

- `frontend/src/hooks/useRunTask.ts`（约 30 行）：
  - `useRunActive`：派生「当前维度是否有进行中任务」，用于提交按钮禁用；
  - `useRunError`：取最近失败任务的错误信息；
  - `guardRunStart`：POST 在途或已有同类任务时以 antd `message.warning` 拦截（防重复入队）。
- `frontend/src/components/common/RunTaskBanner.tsx`（约 55 行，纯展示组件）：仅 `active` / `error` / `activeText` 三个 props——active 为真显示绿色「进行中」横幅（文案 activeText），error 非空显示红色失败横幅。

### 6. AgentRunProgress — Agent 运行进度（SSE 消费）
...
队列事件（2026-09-21 扩展）：`AgentEvent.type` 新增 `'queued'` / `'started'`（含 `queue_position`），对应后端 Agent 运行队列——FIFO、并发上限 `MAX_CONCURRENT_RUNS=5`、run status queued→running→done；agentRunStore 的 `markRunRunning` 由 started 事件驱动。
...
- 签名：`usePolling(callback: () => void | Promise<void>, { interval = 2000, enabled = true, maxPolls?, onMaxPolls? } = {})`。
...
- 次数上限（2026-09-21 新增）：内部 `pollCount` 计数（含首次立即执行）与 `stopped` 标志；`maxPolls` 为最大轮询次数（含首次），达到上限自动停止，并提供 `stop()` 主动停止；达上限自动停止后回调 `onMaxPolls`。
- 调用方：
  - `Step6Videos.tsx` — interval 3000，enabled: 有 pending 视频或后台生成中（hasPendingVideos || isBackgroundGenerating），maxPolls 兜底防无限轮询；
  - `StepEpisodeDesign.tsx` — interval 2000，生成期间轮询分集/实体增量落库；
  - `StepLookbook.tsx` — interval 2000，有活跃 run 或 pending/processing 核心素材图时刷新。

### 11. apiClient — axios 实例与业务 API（`frontend/src/api/client.ts`）
...
- API 清单：
  ...
  - scriptStepApi：updateStoryLogic、adoptStoryLogic（不经 LLM 收敛直接完成第 1 步）、setStoryTitle、updateOutline、generateEpisodes/regenerateEpisode→`data.data.run_id`、listEpisodes、updateEpisode、generateLookbook/completeLookbook/listLookbook（entity_id/task_status query 过滤）/regenerateLookbookImage→run_id/deleteLookbookImage、getLookbookLibrary（素材库分组查询）/importLookbookImage（素材库导入绑定）；
  - entityApi：list（entity_type 过滤）/getReferences；
  - agentRunApi：cancel（POST `/agent-runs/{runId}/cancel`）。
- 已删除（2026-09-21）：assetApi（list/upload/remove，随后端 assets 路由整体移除，见「文件上传与会话资产管理」文档）；stepApi 的 regenerateVideos/regenerateSingleVideo。

### 12. fetchSSE — SSE 流式请求（`frontend/src/api/sse.ts`）
...
- 依赖: antd（Layout/Menu/Sider、Spin、Popconfirm、Tag、Collapse、Empty、Tabs、Modal、message、Badge 等组件与反馈）、zustand（useSessionStore/useScriptSessionStore/agentRunStore）、axios、markmap-lib + markmap-view（^0.18.12）、markdown-it（经 markmap-lib 传递依赖存在于 node_modules，非直接声明，见注意事项）
- 被依赖: MainLayout 消费 SessionSider（两处：视频页 grouped / 剧本页平铺）并全局挂载 AgentRunDock；AgentRunDock 消费 agentRunStore 与 AgentRunProgress；StepOutline/StepEpisodeDesign（剧本侧）与 StepStoryboardOutline/StepSegmentManagement（视频侧）消费 agentRunStore + useRunTask + RunTaskBanner（全局任务跟踪，kind 分别为 outline/episodes/storyboard_outline/segment_prompt）；MindmapView 用于 StepOutline（`frontend/src/components/script/StepOutline.tsx`）与 StepStoryboardOutline（`frontend/src/components/workflow/StepStoryboardOutline.tsx`）；LazyVideo 用于 Step6Videos；usePolling 用于 Step6Videos/StepEpisodeDesign/StepLookbook（见第 10 节）；各业务 Step 组件消费 client.ts 各 API
- 可复用组件: `frontend/src/components/common/index.ts` 统一导出 LazyVideo、MindmapView、PromptViewerModal、RunTaskBanner、SessionSider；hooks（useLazyLoad、usePolling、useRunTask）与工具（client.ts、sse.ts）按模块路径直接引用
...
- **Agent 运行队列语义**：后端运行队列 FIFO、并发上限 `MAX_CONCURRENT_RUNS=5`（backend/core/agent_sdk/registry.py），run status queued→running→done；排队/启动通过 `queued`/`started` 事件（含 queue_position）下发——前端 agentRunStore.markRunRunning 由 started 事件驱动，排队中的任务在 AgentRunDock 中以 queued 状态展示。
...
- **已删除 API**：client.ts 的 assetApi（list/upload/remove）与 stepApi.regenerateVideos/regenerateSingleVideo 已删除，前端无残留调用方；assets 相关能力见「文件上传与会话资产管理」文档。
...
| 2026-09-21 | 新增全局任务跟踪设施（AgentRunDock/agentRunStore/useRunTask/RunTaskBanner，四类 kind，started 事件驱动）；MainLayout 全局挂载任务坞 + 顶栏 ComfyUIConnectionModal 入口 + 会话切换改 SPA 内 loadSession（废弃 navigateWithSession 整页刷新）；usePolling 新增 maxPolls/onMaxPolls/stop；client.ts 新增 settingsApi 与 scriptStepApi 四方法（setStoryTitle/adoptStoryLogic/getLookbookLibrary/importLookbookImage）、删除 assetApi 与 regenerateVideos/regenerateSingleVideo（stepApi 改 importComfyUI/startComfyUIVideo/completeSegment）；术语「定妆照」统一为「核心素材图」 |
