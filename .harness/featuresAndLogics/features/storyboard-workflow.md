---
name: 分镜工作流
摘要: 视频创作工作流第 2/3 步（分镜大纲 + 分镜管理）：Agent 在剧本工作区自主检索生成分镜导图与分镜列表，产物文件化存于 04-storyboards/，支持人工编辑导图、分镜形式/overlap 配置、参考素材图管理（素材池选择 + AI 生成）与 video-prompt skill 分镜提示词生成。
...
- **第 3 步 segment_management 分镜管理**：分镜形式（4 种，仅全能参考模式实现关联逻辑）与 overlap 配置、参考素材图管理（素材池选择弹窗 + AI 生成素材图）、video-prompt skill 生成分镜提示词、显式完成标记（解锁第 4 步视频生成）。
...
- `backend/core/agents/storyboard.py` — 核心业务：`StoryboardWorkflow` 全部逻辑（大纲生成/编辑、分镜配置、素材池、素材图生成、提示词生成、完成标记），业务异常 `StoryboardError`（默认 status_code=400）
...
- `backend/core/agents/workflow_v2.py` — `VideoCreationWorkflowV2`：第 1 步选集（分镜上游）与第 4 步视频生成（`_get_video_segments` 消费分镜文件）
...
- `backend/core/services/video_prompt_skill.py` — `sync_video_prompt_skill`：把 `backend/.claude/skills/video-prompt/` 同步进工作区 `99-references/video-prompt/`（渐进式披露）
- `backend/core/services/workspace_projection.py` — `video_step_results`：会话详情时把分镜文件内容投影回 step_results
...
| GET /{sid}/material-pool | get_material_pool | 素材池分组数据（定妆照/本集素材/其他集素材，不跨 story） |
...
| POST /{sid}/storyboard-segments/{index}/generate-prompt | generate_segment_prompt | video-prompt skill 生成提示词 → {run_id}（label=segment_prompt_{index}，仅全能参考模式） |
| POST /{sid}/segment-management/complete | complete_segment_management | 完成分镜配置，推进到第 4 步 |

Agent 观流统一走 `backend/api/v1/agent_runs.py`（GET /{run_id}/events SSE、GET /{run_id}、POST /{run_id}/cancel）；`StoryboardError` 由 backend/main.py 全局 exception_handler 统一转 HTTP detail。
...
- 读取投影：`video_step_results`（workspace_projection.py）在会话详情接口把 read_storyboard 结果合入 storyboard_outline.result_data（前端凭此渲染）。

### 第 2 步：分镜大纲生成/编辑（storyboard.py）

- `generate_outline`：前置 `ensure_can_execute("storyboard_outline")`；prompt 由模板 `storyboard_outline` 渲染（video_params 上下文 + `_workspace_section` 工作区检索指引 + 用户额外要求 + max_segment_duration）；`run_agent`（system=STORYBOARD_OUTLINE_SYSTEM，tools=READ_ONLY_TOOLS，cwd=story_cwd，max_turns=12）。
...
- **素材池** `list_material_pool`：分组 = 定妆照（story 级 lookbook，key=lookbook）+ 本集素材（key=current）+ 其他集素材（key=episode_{eid}，按 ep_NN 集号排序），均不跨 story；素材图 ID 规则 `lookbook_lb_*` 定妆照 / `mat_*` 分集素材（`_resolve_pool_image` 校验存在、同 story、已生成 image_path）。
...
- `StepStoryboardOutline.tsx`：数据取 `session.step_results.storyboard_outline.result_data`（投影产物）；`buildMindmap` 把 segments 的 outline 拼为 `### 标题` 下的 `- ` 列表子节点（预览模式附加 `Ns · overlap Ns` 后缀，编辑模式不加），编辑保存这份文本由后端解析同步；生成/重生成带额外提示词输入与常用提示词快捷下拉（PRESET_PROMPTS）；有下游完成态时重生成需确认（级联清空提示词与视频数据）；观流用 `AgentRunProgress`（frontend/src/components/script/AgentRunProgress.tsx）。
- `StepSegmentManagement.tsx`：分镜卡片列表，形式下拉（SEGMENT_OPTIONS），全能参考模式时出现 overlap 滑块、参考素材图编辑区与提示词生成入口；提示词弹窗先 GET prompt-context 展示上下文（剧本大纲/本集脚本/当前分镜/参考图/overlap 衔接信息）再发起生成观流；生成中不允许关闭弹窗；已生成提示词支持复制；底部「完成分镜配置」解锁第 4 步。
...
- **上游**：第 1 步 select_episode（workflow_v2.step_select_episode，校验剧本会话 episode_design 已完成）保存 script_session_id/episode_id/episode_title/video_params；`_get_selected` 经 `require_step_data` 读取（未完成抛「会话尚未完成第 1 步：从剧本选集」）。视频会话本身由 POST /api/v1/sessions/from-script（backend/api/v1/sessions.py）创建。
- **下游**：第 4 步 `workflow_v2._get_video_segments` 从 `read_storyboard` 读分镜（content=已生成提示词或分镜大纲，duration=max_segment_duration，reference_images 全能参考图作为该段参考图；http(s) 外链无法上传自动跳过），legacy 会话优先旧 generate_segment_scripts 结果兜底。
...
- 被依赖: backend/api/v1/steps.py（路由层直接调用）；workflow_v2._get_video_segments（第 4 步视频生成读分镜文件）；workspace_projection.video_step_results（会话详情投影）
