...
- **/backend** - FastAPI 后端 API 层（依赖 core 层）
- **/frontend** - React 前端 UI 层（通过 API 与 backend 通信）
...
### 1. 工作流引擎 (`core/agents/workflow_v2.py`)
...
### 2. 会话管理 (`core/persistence/session_manager.py`)
...
1. `SessionManager.STEPS` 列表（`core/persistence/session_manager.py`）
2. `VideoCreationWorkflowV2` 的步骤方法（`core/agents/workflow_v2.py`）
...
### Q: 如何切换视频生成服务？
A: 修改 `.env` 文件中的 `VIDEO_SERVICE_TYPE` 为 `jimeng` 或 `wan22`，或修改 `core/config.py` 中的默认值。

### Q: 会话数据存储在哪里？
A: 会话数据存储在 `data/sessions.db` SQLite数据库中，可安全删除以重置所有会话。

### Q: 如何修改默认视频参数？
A: 修改 `core/config.py` 中的 `DEFAULT_VIDEO_PARAMS` 字典。
